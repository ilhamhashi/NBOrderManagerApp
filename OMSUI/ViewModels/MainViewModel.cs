    ...
    public class MainViewModel : ViewModel
    {
        ...
        public MainViewModel(INavigationService navigationService)
        {
            ...
            NavigateToNewOrderCommand = new RelayCommand(
                execute => { Navigation.NavigateTo<NewOrderViewModel>(); },
                canExecute => true);

            NavigateToOrdersCommand = new RelayCommand(
                execute => { Navigation.NavigateTo<OrdersViewModel>(); },
                canExecute => true);

            NavigateToCustomersCommand = new RelayCommand(
                execute => { Navigation.NavigateTo<CustomersViewModel>(); },
                canExecute => true);

            NavigateToProductsCommand = new RelayCommand(
                execute => { Navigation.NavigateTo<ProductsViewModel>(); },
                canExecute => true);

            NavigateToSalesDataCommand = new RelayCommand(
                execute => { Navigation.NavigateTo<SalesDataViewModel>(); },
                canExecute => true);

            NavigateToHomeCommand = new RelayCommand(
                execute => { Navigation.NavigateTo<HomeViewModel>(); },
                canExecute => true);
        }
    }
}
